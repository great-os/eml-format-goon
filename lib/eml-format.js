...
function matchNextValueLine(line) {
  return /^\s+([^\r\n]+)/g.exec(line);
}
...
emlformat.useEncodingPlugin = function(pluginObj) {
  if (pluginObj &&
    pluginObj.version &&
    pluginObj.encoding &&
    pluginObj.encoder) {
      emlformat.encoderPlugin = pluginObj;
    }
};
...
/******************************************************************************************
 * Set customed encoding and encoder
 * @params eml         EML file content or object from 'parse'
 * @params directory   Folder name or directory path where to unpack
 * @params options     Optional parameters: { parsedJsonFile, readJsonFile, simulate }
 * @params callback    Callback function(error)
 ******************************************************************************************/
emlformat.customEncoder = function(encoding, encoder) {
  emlformat.encoding = encoding;
  emlformat.encoder = encoder;
}
  ...
  function _read(data) {
    try {
      ...
      function _append(headers, content) {
        ...
        if (encoding == "base64") {
          var plugin = emlformat.encoderPlugin;
          if (typeof plugin == 'object' &&
          plugin &&
          plugin.encoding &&
          contentType.toLowerCase().indexOf(plugin.encoding) >= 0) {
            content = plugin.encoder(content)
          } else {
            content = Buffer.from(content.replace(/\r?\n/g, ""), "base64");
          }
          // if (emlformat.encoding && contentType.toLowerCase().indexOf(emlformat.encoding) >= 0) {
          //   content = emlformat.encoder(content, emlformat.encoding)
          // } else {
          //   content = Buffer.from(content.replace(/\r?\n/g, ""), "base64");
          // }
        }
...
function isMultipartContent(ct) {
  return !!ct && /^multipart\//g.test(ct)
}
...
function parseRecursive(lines, start, parent, options) {
  ...
  for (var i = start; i < lines.length; i++) {
    ...
    if (!insideBody) {
      ...
      var match = matchNextValueLine(line);
      if (match) {
        appendValueToHeader(parent, lastHeaderName, isMultiHeader, match);
        continue;
      }

      //Header name and value
      var match = matchHeaderNameAndValue(line);
      if (match) {
        ({ lastHeaderName, isMultiHeader } = addNameValueToHeader(lastHeaderName, match, parent, isMultiHeader));
        continue;
      }
    }
    //Body
      ...
      if (isMultipart) {
        ...
        if (lines[i - 1] == "" && isBoundaryLine(line, findBoundary) && !/--(\r?\n)?$/g.test(line)) {
          insideBoundary = true;
...
function isBoundaryLine(line, findBoundary) {
  return line.indexOf("--" + findBoundary) == 0;
}

function addNameValueToHeader(lastHeaderName, match, parent, isMultiHeader) {
  ...
  return { lastHeaderName, isMultiHeader };
}

function matchHeaderNameAndValue(line) {
  return /^([\w\d\-]+):\s+([^\r\n]+)/gi.exec(line);
}

function appendValueToHeader(parent, lastHeaderName, isMultiHeader, match) {
  var header = parent.headers[lastHeaderName];
  if (isMultiHeader) {
